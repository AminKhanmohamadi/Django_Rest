...
from config.apps.catalog.manager import CategoryQuerySet
from config.libs.db.fields import UpperCaseCharField
from config.libs.db.models import AuditableModel
...
class ProductClass(models.Model):
    tiitle = models.CharField(max_length=255, db_index=True)
    description = models.CharField(max_length=255, null=True, blank=True)
    slug = models.SlugField(unique=True, allow_unicode=True)
...
class ProductAttribute(models.Model):
    ...
    product_class = models.ForeignKey(ProductClass, related_name='attributes', on_delete=models.CASCADE, null=True)
    title = models.CharField(max_length=64)
    type = models.CharField(max_length=255, choices=AttributeTypeChoices.choices, default=AttributeTypeChoices.text)
    option_group = models.ForeignKey(OptionGroup, related_name='options', on_delete=models.PROTECT, null=True,
                                     blank=True)
...
class Option(models.Model):
    ...
    title = models.CharField(max_length=64)
    type = models.CharField(max_length=255, choices=OptionTypeChoices.choices, default=OptionTypeChoices.text)
    option_group = models.ForeignKey(OptionGroup, related_name='attributes', on_delete=models.PROTECT, null=True,
                                     blank=True)
    required = models.BooleanField(default=False)
...
class ProductAttributeValue(models.Model):
    ...
    value_option = models.ForeignKey(OptionGroupValue, on_delete=models.PROTECT, null=True,
                                     blank=True)
